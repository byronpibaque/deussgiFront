...
<v-layout align-start>
    <v-flex>
        ...
        <span class="blue--text" style="font-size: 30px">Porcentajes</span>
        <v-flex xs12 sm12 md12>
          <v-autocomplete :items="congregaciones"
           v-if="$store.state.usuario.rol=='5ff86c9fe04cdc30f4805fe2'"
            v-model="codigoC" v-on:change="listarAdmin(),consultarDatosAdmin()" 
         ...
         <span v-if="bandera" class="green--text" style="font-size: 15px">Las gráficas estan basadas en los valores registrados del tres porciento recaudado.</span>
        <v-flex  xs12 sm12 md12 lg12 xl12>
...
export default {
  ...
  methods: {
    ...
    if(this.$store.state.usuario.rol=="5ff86c9fe04cdc30f4805fe2"){//es administrador
        axios.get('porcentaje/grafico12meses?codigoCongregacion='+me.codigoC,configuracion)
        .then(function (response){
    ...
    if(this.$store.state.usuario.rol!="5ff86c9fe04cdc30f4805fe2"){//no es administrador
      axios.get('porcentaje/grafico12meses?codigoCongregacion='+codeF,configuracion)
      .then(function (response){
          me.valores=response.data;
            me.graficar(codeF);
      }).catch(function(error){
      ...
      me.valores.map(function (x){
        ...
        var myChart = new Chart(ctx, {
            ...
            data: {
                ...
                datasets: [
                  {
                    label: 'Tres % de los ultimos 12 meses',
                    data: me.totales,
                    backgroundColor: [
                        'rgba(255, 99, 132, 0.2)',
                        'rgba(54, 162, 235, 0.2)',
